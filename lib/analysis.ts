...
function computeBiomarkersAndScore(data: BiomarkerData): {
  ...
  if (data.fastingGlucose !== null) {
    ...
    totalScore +=
      status === "optimal" ? 100 : status === "suboptimal" ? 60 : 30;
    count++;
  }
...
function parseStructuredInsights(
  ...
  const keyAreas = Array.isArray(keyAreasIn)
    ? keyAreasIn.filter(isKeyArea)
    : [];
...
function buildRuleBasedInsights(
  ...
  if (
    ...
    keyAreas.push({
      ...
      recommendation:
        "Consider: Reduce refined carbs, add 30min walking after meals",
    });
  }
  ...
  if (data.vitaminD !== null && data.vitaminD < 30) {
    keyAreas.push({
      ...
      recommendation:
        "Consider: 2000-4000 IU daily supplementation, get morning sunlight",
    });
  }
  ...
  if (ldlHigh || trigHigh) {
    ...
    keyAreas.push({
      ...
      recommendation:
        "Consider: Increase omega-3 intake, reduce saturated fats",
    });
  }
...
export async function analyzeData(
  ...
  if (openai) {
    try {
      insights = await fetchOpenAIInsights(
        openai,
        data,
        healthScore,
        biomarkers,
